...
from recommend import get_user_recommendations
th.cuda.empty_cache()
...
def make_HIT_plots(sample_edge_type, HIT_vals, epochs, save_result_dir):
    ...
    plt.title(f'{sample_edge_type} link-pred Train, Val, Test HIT@K')
    plt.xlabel('Epochs')
    plt.ylabel('HIT')
    plt.ylim((0, 1))
    plt.yticks(np.arange(0, 1, step=0.2))
...
def make_DIV_plots(sample_edge_type, DIV_vals, epochs, save_result_dir):
    ...
    plt.title(f'{sample_edge_type} link-pred Train, Val, Test DIV@K')
    plt.xlabel('Epochs')
    plt.ylabel('DIV')
    plt.ylim((0, 1))
    plt.yticks(np.arange(0, 1, step=0.2))
...
def make_COV_plots(sample_edge_type, COV_vals, epochs, save_result_dir):
    zero_pad=[0 for epoch in range(epochs-len(COV_vals['train']))]
    plt.figure()
    plt.plot(range(epochs), smooth(COV_vals['train']+zero_pad), 'r', label='Training COV')
    plt.plot(range(epochs), smooth(COV_vals['val']+zero_pad), 'g', label='Val COV')
    plt.plot(range(epochs), smooth(COV_vals['test']+zero_pad), 'b', label='Test COV')
    plt.title(f'{sample_edge_type} link-pred Train, Val, Test COV@K')
    plt.xlabel('Epochs')
    plt.ylabel('COV')
    plt.ylim((0, 1))
    plt.yticks(np.arange(0, 1, step=0.2))
    plt.legend()
    plt.savefig(save_result_dir+f"/{sample_edge_type}_cov_plot.png")
...
def train_model(model, optimizer,scheduler, train_loader, val_loader, test_loader, save_folder, sampled_edge_type, date, args):
    ...
    for epoch in tqdm_loader:
        ...
        train_HIT, train_DIV, train_COV = evaluate_link_recommendation(
            predict_scores=train_y_predicts, 
            true_scores=train_y_trues,
        ...
        COV_vals['train'].append(train_COV)
        ...
        val_HIT, val_DIV, val_COV = evaluate_link_recommendation(
            predict_scores=val_y_predicts, 
            true_scores=val_y_trues,
        ...
        test_HIT, test_DIV, test_COV = evaluate_link_recommendation(
            predict_scores=test_y_predicts, 
            true_scores=test_y_trues,
        ...
        COV_vals['test'].append(test_COV)

        if best_validate_RMSE is None or val_RMSE < best_validate_RMSE:
            best_validate_RMSE = val_RMSE
            scores = {"RMSE": float(f"{test_RMSE:.4f}"), "MAE": float(f"{test_MAE:.4f}"),"AUC": float(f"{test_AUC:.4f}"), "AP": float(f"{test_AP:.4f}"), 'HIT': float(f"{test_HIT:.4f}"), 'DIV': float(f"{test_DIV:.4f}"), 'COV': float(f"{test_COV:.4f}")}
            final_result = json.dumps(scores, indent=4)

        tqdm_loader.set_description(f'EPOCH #{epoch} RMSE: {test_RMSE:.2f}, MAE: {test_MAE:.2f}, AUC: {test_AUC:.2f}, AP: {test_AP:.2f}, HIT: {test_HIT:.2f}, DIV: {test_DIV:.2f}, COV: {test_COV:.2f} ')
    ...
    make_COV_plots(sampled_edge_type, COV_vals, args.epochs, save_result_dir)
...
def train_models(hg, args):
    ...
    for sampled_edge_type in [etype for _,etype,_ in hg.canonical_etypes]:
        if sampled_edge_type in ['listened_to_track','listened_to_album','listened_to_artist']:
            found_sampled_edge_types.append(sampled_edge_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--shuffle',  default=True, type=str2bool, nargs='?', const=True, help='DEPRECATED DO NOT USE')
    ...
    parser.add_argument('--rel_input_dim', default=8, type=int, help='input dimension of the edges')
    ...
    parser.add_argument('--dropout', default=0.2, type=float, help='the dropout rate for the models')
    ...
    parser.add_argument('--patience', default=1, type=int, help='the number of epochs to allow before early stopping')
    parser.add_argument('--split_by_users', default=False, type=str2bool, nargs='?', const=True, help='DEPRECATED Do not use')
    ...
    parser.add_argument('--device', default='cuda', type=str, help='GPU or CPU device specification for model allocation')
    ...
    parser.add_argument('--playcount_weight', default=True, type=str2bool, nargs='?', const=True, help='indication give every edge a "playcount weight" feature, or every edge with "timestamp" features between a user and their unique listen events')
    parser.add_argument('--norm_playcount_weight', default=False, type=str2bool, nargs='?', const=True, help='indication give every edge a "normalized playcount weight" feature, or "total playcount weight"')
    ...
    parser.add_argument('--emb_dim', default=8, type=int,  help='node embedding vector size')
    ...
    parser.add_argument('--metapath2vec_epochs', default=3, type=int,  help='epochs of metapath2vec')
    ...
    dataset = LFM1b(
        ...
        device=args.device, # DEPRECATED THIS DOES NOT AFFECT LFM-1b (the data loader is always in CPU)
